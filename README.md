...
It's also necessary to create the DB table that will hold the feature flag data. To do that, [create a new migration](https://hexdocs.pm/ecto_sql/Mix.Tasks.Ecto.Gen.Migration.html) in your project and copy the contents of the provided migration files, in order: [the base table](https://github.com/tompave/fun_with_flags/blob/master/priv/ecto_repo/migrations/00000000000000_create_feature_flags_table.exs), [the timestamps](https://github.com/tompave/fun_with_flags/blob/master/priv/ecto_repo/migrations/00000000000001_add_timestamps.exs), and [the metadata column](https://github.com/tompave/fun_with_flags/blob/master/priv/ecto_repo/migrations/00000000000002_add_metadata.exs). Then [run the migrations](https://hexdocs.pm/ecto_sql/Mix.Tasks.Ecto.Migrate.html).

The `metadata` column stores an optional JSON-compatible map, passed as the `:metadata` option to `FunWithFlags.enable/2` and `FunWithFlags.disable/2`, that records who made an override change (and any other context). Because it is stored as JSON, keys are returned as strings on read (prefer string keys, e.g. `%{"who" => "adam"}`) and values must be JSON-encodable. A metadata-less `enable`/`disable` preserves any previously stored metadata. It is only supported by the Ecto adapter; the Redis adapter ignores it.

Metadata is serialized as JSON, so it relies on the JSON library your Ecto driver is configured to use (e.g. [Jason](https://hex.pm/packages/jason)) — the same one a Postgres/MySQL/SQLite Ecto setup already requires. FunWithFlags itself only depends on `:jason` as a `dev`/`test` dependency, for its own SQLite test suite.
